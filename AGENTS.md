...
- `.env.example`: environment variable template for local/dev deployment.
- `internal/config`: configuration loading from defaults and environment variables.
...
Use `OPENCODE_API_KEY`, `BASE_URL`, `NON_STREAM_TIMEOUT_MS` (or legacy alias `TIMEOUT_MS`), `LISTEN_PORT`, and `MODELS_NEED_TRANSFORMATION` to configure runtime behavior.
...
Never commit real `.env` files, API keys, or local shell exports. Keep `.env.example` placeholder-only. Treat request and response logs as sensitive because prompts, tool payloads, and model outputs may contain private data.
